...
try:
    ...
    print("Warning: 'onewire' or 'ds18x20' library not found. DS18B20 sensor will not work.")
...
def _read_ds18b20_bus(config):
    ...
    try:
        ...
        for rom in roms:
            ...
            sensor_id = f"{config['provides']['temperature']['id_prefix']}_{rom_id[-4:]}"
...
def read_all_sensors():
    ...
    for sensor_name, config in SENSORS.items():
        ...
        #     all_readings.extend(_read_soil_moisture(config))
            ...
            print(f"Warning: No reader function implemented for sensor type '{config['type']}'")
